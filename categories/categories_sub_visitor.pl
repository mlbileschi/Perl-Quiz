...
system("wget", "http://en.wikipedia.org/wiki/Category:".$super_category, "-O", "wikipedia_macro_category_article_$super_category.html", "-q");
...
foreach my $tmp_arr (@subcategories_to_visit)
{
	...
	system("wget", $subcategory_url, "-O", "wikipedia_category_article_$super_category.html", "-q");
	...
	foreach (@articles_to_visit)
	{
		...
		system("wget", $_, "-O", "wikipedia_category_article_$super_category.html", "-q");
		open(INFILE, "<wikipedia_category_article_$super_category.html") or die "couldn't open file wikipedia_category_article_$super_category.html\n";
		...
		foreach my $sentence (@sentences)
		{
			...
			if($line[0]=~/^[A-Za-z]+[,\.\?\!]?$/)
			{
				...
				else
				{
					...
					system("wget", $url, "-q", "-O", "wiktionary_lookup_file_$super_category.html");
	...
	open(OUTFILE, ">$super_category/$subcategory_filename") or die "couldn't open category file";
