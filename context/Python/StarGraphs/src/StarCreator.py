...
def newStar(filename, query):
		...
		print("start file " + filename + ", query " + query);
		...
		for para in inputFileParagraphs:
			...
			for word in para:
				if(onlyAlpha(word) == ''):
					pass #sentence.remove(onlyAlphaword)
				else:
					inputWordCount+=1
			...
			if(len(sentence)!=0 and sentence[0]!=""):
				if(sentence[0].lower() in dictionary and not sentence[0] in dictionary):
					if(not re.match(dictionary[sentence[0].lower()][0],'proper')):
						...
						if(not sentence[0].lower() == sentence[0]):
							try:
		...
		for i in range(0,int(len(spokes))-1):
			#we define 50 as our cutoff point 
			if(spokes[i][1]<50):
				break
